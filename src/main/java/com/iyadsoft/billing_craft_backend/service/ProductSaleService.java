...
public class ProductSaleService {
    ...
    // Double totalVat =
    // customerRepository.findTotalVatByCustomerName(customer.getCName(),
    ...
    // productStockRepository.findById(salesItemDTO.getProId())
    // .orElseThrow(
    // () -> new RuntimeException("ProductStock not found for proId: " +
    // salesItemDTO.getProId()));

    // totalValue += (salesItemDTO.getSprice() - salesItemDTO.getDiscount() -
    ...
    // String phoneNumber =
    // retailerInfoRepository.findByRetailerNameAndUsername(customer.getCName(),
    // customer.getUsername());
    ...
    // phoneNumber,
    // "Dear " + savedCustomer.getCName() + ", your total bill is Tk " +
    // formattedTotalValue
    // + ". And total due is Tk "
    ...
    public List<ProductSale> processSales(SalesRequest saleRequest) {
        ...
        // Prepare to save each ProductSale item
        List<ProductSale> savedSalesItems = new ArrayList<>();
        List<Long> soldProIds = new ArrayList<>(); // ← Collect proIds to delete from ProductOrder
        ZonedDateTime dhakaTime = ZonedDateTime.now(ZoneId.of("Asia/Dhaka"));
        double totalValue = 0;
        ...
        for (SalesItemDTO salesItemDTO : saleRequest.getSalesItems()) {
            ...
            soldProIds.add(salesItemDTO.getProId());
        }

        // 🔥 Delete ProductOrders after sale
        if (!soldProIds.isEmpty()) {
            productOrderRepository.deleteByProIdIn(soldProIds);
        }
        ...
        String smsResponse = smsService.sendSms(
                ...
                "Dear " + savedCustomer.getCName() + ", your total bill is Tk " + formattedTotalValue
                        + ". And total due is Tk " + formattedTotalDue + ". Thanks from " + savedCustomer.getUsername()
                        + ".");
    ...
    public Optional<SaleEditDto> getSaleInfoByProductno(
            String username,
            String productno) {

        List<SaleEditDto> result = productSaleRepository.findSaleinfoByUsernameAndProductno(
                username,
                productno,
                PageRequest.of(0, 1));
    ...
    public ProductSale updateSaleInfo(Long saleId, ProductSale updatedData) {

        ProductSale existing = productSaleRepository.findBySaleId(saleId)
                .orElseThrow(() -> new RuntimeException("Sale info not found for saleId: " + saleId));

        // update fields
        existing.setSaleNote(updatedData.getSaleNote());
        existing.setSprice(updatedData.getSprice());
        existing.setDiscount(updatedData.getDiscount());
        existing.setOffer(updatedData.getOffer());
        existing.setDate(updatedData.getDate());

        return productSaleRepository.save(existing);
    }

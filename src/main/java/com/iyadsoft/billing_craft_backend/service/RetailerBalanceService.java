...
public class RetailerBalanceService {
    ...
    public RetailerBalanceService(RetailerInfoRepository retailerInfoRepository, ProductSaleRepository productSaleRepository, RetailerPaymentRepository retailerPaymentRepository) {
   ...
   public List<RetailerBalanceDto> getRetailerBalance(String username) {
    ...
    return resultMap.values().stream()
            .filter(dto -> dto.getTotalProductValue() > 0 || dto.getCurrentPaymentTotal() > 0 || dto.getPreviousPaymentTotal() > 0)
 ...
 public List<RetailerDetailsDto> getRetailerDetails(String username, String retailerName) {
        ...
        List<RetailerDetailsDto> productSales = productSaleRepository.findProductSalesByUsernameAndReatilerName(username, retailerName);
        List<RetailerDetailsDto> payments = retailerPaymentRepository.findDetailsPaymentByUsernameAndRetailerName(username, retailerName);
